...
export const ConnectButtonModule = (
  ...
  const mapRdtConnectButtonStatus = (
    status: 'success' | 'fail' | 'error' | 'pending' | 'default',
  ): ConnectButtonStatus =>
    status === 'success' ? 'success' : status === 'fail' ? 'error' : 'pending'

  const getPendingOrDefaultConnectButtonStatus = () =>
  ...
  subscriptions.add(
    merge(
      ...
      getPendingOrDefaultConnectButtonStatus().pipe(
      ...
      .pipe(
        ...
        map(([prev, curr]) => ({
          prev: mapRdtConnectButtonStatus(prev),
          curr: mapRdtConnectButtonStatus(curr),
        })),
        switchMap(({ prev, curr }) => {
